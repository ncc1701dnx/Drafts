...
######################################################################################################################################
## 1：现在输入这个自动做表的函数：
IBDMDB_MBX_check <- function(formula, popular = T, wilcox = F, ppm = 10){
    ...
    for(i in 1:length(tt)){
      ...
      if(length(which(!is.na(control_mm))) < 4 | length(which(!is.na(treatment_mm))) < 4) {
        ...
        if(wilcox == F){
          ...
          fold_change <- round(mean(treatment_mm)/mean(control_mm), digits = 1)
        } else if(wilcox == T){
          # wilcox.if.new.users
...
IBDMDB_MBX_add_signif <- function(IBDMDB_MBX_check_result){
  IBDMDB_MBX_check_result$signif <- ifelse(IBDMDB_MBX_check_result$Fold_changes >= 1.5 & -log10(IBDMDB_MBX_check_result$P_Values) > 2, 
                                           "Significantly Abundant", "No Significance")
}
## 开始写自动化作图的函数
IBDMDB_MBX_plot <- function(IBDMDB_MBX_check_result, compound){
  ggplot(na.omit(IBDMDB_MBX_check_result), aes(x = Fold_changes, y = -log10(P_Values), col = significance)) +
    geom_point(size = 3) +
    scale_colour_brewer(palette = "Set2")+
    xlab("Difference in medians (Fold Change)") +
    ylab("-Log10 P-value") +
...
IBDMDBresults_boxplot <- function(IBDMDB_compound, compound_name){
  ...
  for(i in 1:nrow(rr_result)){
    ...
    my_colors <- c("darkorchid4", "cornflowerblue")
