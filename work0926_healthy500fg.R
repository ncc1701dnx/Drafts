...
GENE_FG <- read.table("Healthy_500FG/dehydrogenae/P_vulgatus_3_oxo_5_alpha_steroid_4_dehydrogenae.faa.Healthy_500FG.70cov_30id.blastp.out.FG_ann", sep = "\t", header = T, check.names = F)

dim(GENE_FG)
dim(GENE_msp)

GENE_M[(which(GENE_M$genus == "CAG-485")),]
## glucuronidae
GENE_msp <- read.table("Healthy_500FG/glucuronidae/P_vulgatus_beta-glucuronidae.faa.Healthy_500FG.70cov_30id.blastp.out.msp_ann", sep = "\t", header = T, check.names = F)
GENE_rpkm <- read.table("Healthy_500FG/glucuronidae/P_vulgatus_beta-glucuronidae.faa.Healthy_500FG.70cov_30id.blastp.out.rpkm", sep = "\t", header = T, check.names = F)
GENE_FG <- read.table("Healthy_500FG/glucuronidae/P_vulgatus_beta-glucuronidae.faa.Healthy_500FG.70cov_30id.blastp.out.FG_ann", sep = "\t", header = T, check.names = F)

dim(GENE_FG)
dim(GENE_msp)

GENE_M[which(GENE_M$genus == "UBA7173"),]
...
selected_meta_healthy500fg <- meta_healthy500fg %>%
  select(sample_id, gender)

filtered_data_GENE <- filtered_data_GENE %>%
  left_join(selected_meta_healthy500fg, by = c("sample_id" = "sample_id"))
...
table(filtered_data_GENE$gender)
...
custom_colors <- c(
  ...
  "Prevotella" = "#20793c",
  "Ruminococcus" = "#DADEB5",
  "Muribaculaceae" = "darkorange3",
  "others" = "azure4"
)
genus_rpkm_sum$genus[which(genus_rpkm_sum$genus == "Ruminococcus_C")] <- "Ruminococcus"
genus_rpkm_sum$genus[which(genus_rpkm_sum$genus == "UBA7173")]<- "Muribaculaceae"
